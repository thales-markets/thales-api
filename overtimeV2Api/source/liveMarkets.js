...
async function processAllMarkets(network) {
  const liveMarketsToSave = await getOpenLiveMarkets(network);

  if (Array.isArray(liveMarketsToSave)) {
    redisClient.set(KEYS.OVERTIME_V2_LIVE_MARKETS[network], JSON.stringify(liveMarketsToSave), function () {});
  }
}

function getOpenLiveMarkets(network) {
    ...
    redisClient.get(KEYS.OVERTIME_V2_OPEN_MARKETS[network], async function (err, obj) {
      try {
        ...
        if (filteredMarkets.length > 0) {
            ...
            if (responseObject != undefined) {
              ...
              if (
                ...
                if (aggregationEnabled > 0) {
                  ...
                  return adjustSpreadAndReturnMarketWithOdds(
                    market,
                    spreadData,
                    primaryBookmakerOdds,
                    LIVE_TYPE_ID_BASE,
                  );
                }
              }
          ...
          const openLiveMarkets = filteredMarketsWithLiveOddsAndDummyMarkets.filter((market) => market != null);
          resolve(openLiveMarkets);
        } else {
          resolve([]);
        }
